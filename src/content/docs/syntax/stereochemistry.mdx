...
<Aside type="note" title="Atom-level stereo">
  Per-atom chirality (R/S designation) is now supported via the `@R`
  and `@S` suffixes. See [Atoms](./atoms/) for the full field list.
  ...
  These map to CML `aci:atomParity` on the `<atom>` element.
  Wedge/hash bonds (`>-`/`-<`) carry bond stereo via CML
  `<bondStereo>`.
</Aside>
