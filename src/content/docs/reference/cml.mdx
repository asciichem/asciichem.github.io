...
These features exist in the `chemicalml` gem's canonical model and
wire layer but have no AsciiChem text syntax yet:

- **Fractional coordinates.** The canonical `Atom` carries
...
- **Spin multiplicity.** The canonical `Atom` has a
  `spin_multiplicity` field for quantum-chemistry multiplicity
  (singlet/doublet/triplet). AsciiChem has no syntax for this yet.
