...
The canonical model is currently narrower than the AsciiChem model.
Some gaps are closed via the `aci:` extension namespace; others are
still open. Each gap is guarded by a spec so the loss is explicit, not
silent.

### Supported by `chemicalml` but not yet wired from AsciiChem

The `chemicalml` gem (v0.2.0) now supports these natively in the
canonical model and wire layer; AsciiChem's adapter doesn't yet
produce or consume them:

- **Atom coordinates.** The canonical `Atom` carries `x2`/`y2`,
  `x3`/`y3`/`z3`, and `xFract`/`yFract`/`zFract`. AsciiChem's text
  syntax has no coordinate notation yet (planned via the 2D structural
  renderer).
- **Stereochemistry.** The canonical model has `AtomParity` (with
  `atom_refs4` + value) and `BondStereo` (with `atom_refs2` /
  `atom_refs4` + value). AsciiChem's `(R)-` / `(S)-` / `(E)-` / `(Z)-`
  prefixes parse into the AsciiChem model but don't yet map through
  to `<atomParity>` / `<bondStereo>` elements.
- **Molecule metadata.** The canonical `Molecule` has `names`,
  `identifiers`, `formulas`, `properties`, and `labels` as
  first-class children. AsciiChem's model has `Name` and `Identifier`
  structs but the grammar doesn't yet parse them from text.
- **Compchem modules.** `<module convention="convention:compchem">`-
  rooted documents parse and round-trip through the canonical model;
  AsciiChem has no compchem syntax.

### Still open

- **Reaction cascades.** `A -> B -> C` produces a `ReactionList` in
  the canonical model but the CML wire layer doesn't yet serialise
  `<reactionList>` at the document root.
- **Spectroscopy, crystallography, polymer notation.** CML has
  extensive support (the gem models all 121 elements); AsciiChem
  doesn't have syntax for these.
