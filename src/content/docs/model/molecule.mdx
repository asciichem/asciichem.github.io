---
title: Molecule
description: An ordered sequence of atoms and groups with optional coefficient and stereochemistry marker.
---

# Molecule

A molecule is an ordered list of atoms, groups, and bonds, with an
optional leading stoichiometric coefficient and an optional
stereochemistry marker.
...
The `stereo_prefix` is optional; if present it must be a closed-letter
...
The chemist reads `(R)-CH(OH)COOH` and knows which carbon is chiral
...
So stereo is metadata on the molecule. A future 2D structural renderer
(via elk-rb integration) will be able to attach the marker to the
specific atom via graph analysis; for now, the molecule-level marker
...
The linter walks into a molecule's nodes via `children`. Adding a new
